...
nutilde = max(nutilde, 0);  % Forcer nutilde à être >= 0
nutilde(isinf(nutilde)) = 0; % Remplace les Inf par 0
nutilde(isnan(nutilde)) = 0; % Remplace les NaN par 0
% apply bc for lid-driven cavity.
nutilde(1,:) = 0;
nutilde(end,:) = 0;
nutilde(:,1) = 0;
...
nut = nutilde.*fv1;
nut = max(nut, 0);  % Force nut à être >= 0
nut(isinf(nut)) = 0; % Remplacer les Inf par 0
nut(isnan(nut)) = 0; % Remplacer les NaN par 0
